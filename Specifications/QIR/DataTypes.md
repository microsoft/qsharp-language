...
#### Callable Values and Wrapper Functions
...
Each wrapper is an LLVM function that takes three tuple pointers as input and
returns no output; that is, `void(%TupleHeader*, %TupleHeader*, %TupleHeader*)`.
The first input is the capture tuple, which used for lambdas and partial
applications.
...
That is, a callable that is never turned into a callable value doesn't need
either wrapper functions or an implementation table.
...
*TODO*: Should we keep this section?
...
1. Create a tuple from the current values of `x` and `y`. 

2. Call the `callable_create` library routine with the implementation table for the
   compiler-generated callable and the pointer to the tuple allocated in step 1.

3. Assign the return from the `callable_create` call to the local variable `%f`.
