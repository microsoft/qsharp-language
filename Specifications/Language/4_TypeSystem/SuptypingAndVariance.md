...
Purely based on the fact that arrays are immutable in Q#, it follows that they should be covariant. In reality, they are in fact invariant. This is a consequence of the current implementation rather than a strict necessity, and it may be possible to revise that in the future; it is an artifact stemming from the data type that arrays are compiled into, which supports in-place modification for optimization purposes. 
